...
namespace Pet
{
    ...
    public partial class ProductForm : Form
    {
        ...
        private void dgvProduct_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if(MessageBox.Show("Are you sure you want to delete this items?", "Delete Record", MessageBoxButtons.YesNo,
                    MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    dbcon.executeQuery("DELETE FROM tbProduct WHERE pcode LIKE '" +
                        dgvProduct.Rows[e.RowIndex].Cells[1].Value.ToString() + "'"); //using dbcon to shorten code

                    MessageBox.Show("Item record has been successfully removed!", title,
        ...
        public void LoadProduct()
        {
            ...
            cm = new SqlCommand("SELECT * FROM tbProduct WHERE CONCAT(pname, ptype, pcategory) " +
