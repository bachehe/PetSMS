...
namespace Pet
{
    public partial class CashProduct : Form
    {
        ...
        private void btnCash_Click(object sender, EventArgs e)
        {
            foreach (DataGridViewRow dr in dgvProduct.Rows)
            {
                ...
                if (chkbox)
                {
                    try
                    {
                        cm = new SqlCommand("INSERT INTO tbCash(transno, pcode, pname, qty, price, cashier) " +
        ...
        public void LoadProduct()
        {
            ...
            cm = new SqlCommand("SELECT pcode, pname, ptype, pcategory, pprice FROM tbProduct" +
                " WHERE CONCAT(pname,ptype,pcategory) LIKE '%" + txtSearch.Text + "%' AND pqty > " + 0 + "", cn);
            cn.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
